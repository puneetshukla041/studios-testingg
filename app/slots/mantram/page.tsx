...
export default function MantramSlotPage() {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/bookings/mantram', {
        method: 'POST',
  ...
  const handleDownloadTicket = async () => {
    ...
    try {
      ...
      link.download = `Mantram-Ticket-${confirmedBooking?.sequentialId || 'SSI'}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
      console.error(err);
            ...
            <form onSubmit={handleFormSubmit} className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm space-y-6">
              ...
              <div className="flex justify-end items-center space-x-3 pt-4 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="px-6 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-semibold text-sm"
                >
